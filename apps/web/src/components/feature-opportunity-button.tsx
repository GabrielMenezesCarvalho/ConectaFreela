...
import { apiFetch, type FeatureOpportunityResult, type Opportunity } from "@/lib/api";

export function FeatureOpportunityButton({ opportunity, organizerUserId, isPremium, remainingCredits, onFeatured }: { opportunity: Opportunity; organizerUserId: string; isPremium: boolean; remainingCredits: number; onFeatured: (opportunity: Opportunity, remainingCredits: number) => void }) {
  ...
  if (remainingCredits < 1) return <Link className="flex items-center gap-1.5 rounded-lg border border-amber-300 bg-amber-50 px-4 py-2 text-sm font-bold text-amber-800" href="/organizacao/premium/assinar"><Crown className="size-4" /> Comprar 3 destaques</Link>;

  async function confirm() {
    setPending(true); setError("");
    try {
      const result = await apiFetch<FeatureOpportunityResult>(`/opportunities/${opportunity.id}/feature`, { method: "PATCH", body: JSON.stringify({ organizerUserId }), fallbackError: "Não foi possível destacar a oportunidade." });
      onFeatured(result.opportunity, result.remainingFeaturedCredits); setOpen(false);
  ...
  return <>
    ...
    {open && <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/60 p-5" role="presentation" onMouseDown={() => setOpen(false)}>
      <div className="w-full max-w-md rounded-2xl bg-white p-6 shadow-2xl" role="dialog" aria-modal="true" aria-labelledby="feature-title" onMouseDown={(event) => event.stopPropagation()}>
        ...
        <div className="mt-4 flex items-center justify-between rounded-xl bg-amber-50 px-4 py-3 text-sm text-amber-900"><span>Esta ação consome 1 crédito</span><strong>{remainingCredits} → {remainingCredits - 1}</strong></div>
