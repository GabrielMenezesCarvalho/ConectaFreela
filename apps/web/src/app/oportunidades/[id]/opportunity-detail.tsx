...
import { ArrowLeft, BriefcaseBusiness, CheckCircle2, Clock, FileText, MapPin, Send, Sparkles, Users, X, MessagesSquare } from "lucide-react";
...
function ApplicationSummary({ application, unreadCount, pending, confirmWithdraw, onAskWithdraw, onCancelWithdraw, onWithdraw }: { application: Application; unreadCount: number; pending: boolean; confirmWithdraw: boolean; onAskWithdraw: () => void; onCancelWithdraw: () => void; onWithdraw: () => void }) {
  return <div><span className={`inline-flex rounded-full border px-3 py-1 text-xs font-semibold ${applicationStatusStyles[application.status]}`}>{applicationStatusLabels[application.status]}</span><h2 className="mt-4 text-xl font-bold">Candidatura enviada</h2><p className="mt-2 text-sm text-slate-500">Enviada em {dateFormatter.format(new Date(application.createdAt))}</p><div className="mt-5 rounded-xl bg-slate-50 p-4"><p className="text-xs font-semibold uppercase tracking-wide text-slate-400">Sua mensagem</p><p className="mt-2 text-sm leading-6 text-slate-600">{application.message}</p></div>{application.conversation ? <><Link className="mt-5 flex w-full items-center justify-center gap-2 rounded-xl bg-emerald-700 px-5 py-3 font-bold text-white" href={`/mensagens/${application.conversation.id}`}><MessagesSquare className="size-4" /> Abrir conversa{unreadCount > 0 && <span className="ml-1 inline-flex min-w-6 items-center justify-center rounded-full bg-white px-2 py-0.5 text-xs font-bold text-emerald-800">{unreadCount > 99 ? "99+" : unreadCount}</span>}</Link>{unreadCount > 0 && <p className="mt-2 text-center text-xs font-semibold text-emerald-700">{unreadCount === 1 ? "1 mensagem nova" : `${unreadCount} mensagens novas`}</p>}<Link className="mt-3 flex w-full items-center justify-center gap-2 rounded-xl border border-slate-200 px-5 py-3 text-sm font-bold text-slate-700" href="/candidaturas"><FileText className="size-4" /> Acompanhar candidaturas</Link></> : <Link className="mt-5 flex w-full items-center justify-center gap-2 rounded-xl bg-emerald-700 px-5 py-3 font-bold text-white" href="/candidaturas"><FileText className="size-4" /> Acompanhar candidaturas</Link>}{application.status === "UNDER_REVIEW" && (confirmWithdraw ? <div className="mt-4 rounded-xl border border-red-200 bg-red-50 p-4"><p className="text-sm text-red-800">Tem certeza? O envio não poderá ser refeito.</p><div className="mt-3 flex gap-2"><button className="flex-1 rounded-lg border border-red-200 bg-white px-3 py-2 text-sm font-semibold" onClick={onCancelWithdraw} type="button">Cancelar</button><button className="flex-1 rounded-lg bg-red-600 px-3 py-2 text-sm font-semibold text-white disabled:opacity-60" disabled={pending} onClick={onWithdraw} type="button">{pending ? "Retirando..." : "Confirmar"}</button></div></div> : <button className="mt-3 flex w-full items-center justify-center gap-2 px-4 py-2 text-sm font-semibold text-red-600" onClick={onAskWithdraw} type="button"><X className="size-4" /> Retirar candidatura</button>)}</div>;
}
